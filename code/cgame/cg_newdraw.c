...
static void CG_DrawPlayerArmorIcon( rectDef_t *rect, qboolean draw2D ) {
  ...
  if ( cg_drawStatus.integer == 0 ) {
	...
	if ( draw2D || ( !cg_draw3dIcons.integer && cg_drawIcons.integer) ) {
		CG_DrawPic( rect->x, rect->y + rect->h/2 + 1, rect->w, rect->h, CG_GetArmorIcon() );
		...
		CG_Draw3DModel( rect->x, rect->y + rect->h/2 + 1, rect->w, rect->h, CG_GetArmorModel(), 0, origin, angles );
	}
}
...
void CG_ShowResponseHead() {
	...
	if (menu) {
		...
		menu->window.rect.x = cgs.screenXmin + 5;
		menu->window.rect.y = cgs.screenYmin + 5;
		// Update all item positions within the menu after repositioning
		Menu_UpdatePosition(menu);
	}
	Menus_OpenByName("voiceMenu");
	// Set console X offset so notification text appears next to the voice head
	// cl_conXOffset is in screen pixels, so we need to convert from virtual coords
	// The voice head is at screenXmin + 5 + ~55 virtual units wide = ~60 from screen edge
	// Convert to screen pixels: 60 virtual units * screenXScale
	trap_Cvar_Set("cl_conXOffset", va("%i", (int)(60.0f * cgs.screenXScale)));

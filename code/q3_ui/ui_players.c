...
static void UI_PlayerAngles( playerInfo_t *pi, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	pi->viewAngles[YAW] = 180 + (45.0f * sin(DEG2RAD(AngleNormalize360(uis.realtime / 40.0f))));
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time ) {
	...
	// for colored skins
	memset( legs.shaderRGBA.rgba, 255, sizeof( legs.shaderRGBA.rgba ) );
	...
	// for colored skins
	memset( torso.shaderRGBA.rgba, 255, sizeof( torso.shaderRGBA.rgba ) );
	...
	// for colored skins
	memset( head.shaderRGBA.rgba, 255, sizeof( head.shaderRGBA.rgba ) );

...
static qboolean ValidVoteCommand( int clientNum, char *command ) 
{
	...
	*s = '\0';
	...
	if ( Q_stricmp( buf, "g_mode" ) == 0 ) {
		if ( !Q_stricmp( command, "vq3" ) || !Q_stricmp( command, "q3" ) ) i = MODE_VQ3;
		else if ( !Q_stricmp( command, "cpm" ) || !Q_stricmp( command, "cpma" ) ) i = MODE_CPM;
		else if ( !Q_stricmp( command, "ql" ) ) i = MODE_QL;
		else if ( !Q_stricmp( command, "qlt" ) ) i = MODE_QLT;
		else {
			i = atoi( command );
			if ( i < MODE_VQ3 || i >= MODE_COUNT ) {
				trap_SendServerCommand( clientNum, va( "print \"Invalid mode: %s.\n\"", command ) );
				return qfalse;
			}
		}
		BG_sprintf( base, "g_mode %i", i );
		return qtrue;
	}
